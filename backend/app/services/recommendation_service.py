...
class RecommendationService:
    ...
    async def generate_outfit(self, user_id: UUID, event_raw: str, event_json: dict) -> Dict:
        ...
        try:
            ...
            scored_items = await self._score_items_for_event(item_descriptions, event_context)
            
            # 6. Gerar outfit com múltiplas tentativas
            outfit_result = await self._generate_outfit_with_retries(
                event_raw, event_context, scored_items, user_preferences
            )
            ...
            final_analysis = await self._analyze_final_outfit(
                event_raw, event_json, outfit_items_full, validation_result
            )
    ...
    async def _score_items_for_event(self, items: List[Dict], event_context: Dict) -> List[Dict]:
    ...
    async def _generate_outfit_with_retries(self, event_raw: str, event_context: Dict, scored_items: List[Dict], user_preferences: Dict) -> Dict:
        ...
        for strategy in strategies:
            try:
                outfit = await self._try_outfit_generation(event_raw, event_context, categories_available, user_preferences, strategy)
    ...
    async def _try_outfit_generation(self, event_raw: str, event_context: Dict, categories_available: Dict, user_preferences: Dict, strategy: str) -> List[str]:
    ...
    def _generate_fallback_outfit(self, categories_available: Dict) -> Dict:
        ...
        return {"error": "Não foi possível gerar outfit"}
    ...
    async def _analyze_final_outfit(self, event_raw: str, event_json: dict, outfit_items: List[Item], validation_result: Dict) -> str:
